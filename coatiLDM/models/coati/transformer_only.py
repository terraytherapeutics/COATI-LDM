...
from coatiLDM.models.coati.smiles_xformer import (
...
class COATI_Smiles_Inference(nn.Module):
    ...
    def hcoati_to_2d_batch_beam(
        ...
        assert h_token.dim() == 2
        assert h_token.shape[-1] == self.xformer.n_embd
        generation, probs = self.xformer.generate_beam_search_batch(
            prefix=token_prebatch,
            predictions=max_len,
            beam_width=beam_width,
            batch_size=beam_iter_batch,
            inj_token=tokenizer.unk_token,
            inj_payload=h_token,
            stop_token=tokenizer.stop_token,
        ...
        smiles_list = [
            [
                tokenizer.decode(token_out, special=keep_special)
                for token_out in generation[i].tolist()
            ]
            for i in range(len(generation))
        ]
        tore = {"smiles": smiles_list}
        if return_tokens:
            tore["tokens"] = generation
        if return_probs:
            tore["probs"] = probs
