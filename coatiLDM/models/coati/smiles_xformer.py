...
from coatiLDM.models.coati.basic_transformer import (
    RotaryEmbedding,
    RotaryBlock,
)
...
class RotarySmilesTransformer(nn.Module):
    ...
    def generate_beam_search_batch(
        ...
        next_probabilities = self.xformer_blocks(
            prefix_x, apply_norm=True, output_logits=True
        )[:, -1, :]
        vocabulary_size = next_probabilities.shape[-1]
        probabilities, idx = next_probabilities.log_softmax(-1).topk(
            k=beam_width, dim=-1
        )
        ...
        for i in predictions_iterator:
            ...
            next_probabilities = next_probabilities.view(
                -1, beam_width, vocabulary_size
            )
            ...
            best_candidates += (
                torch.arange(X.size(0) // beam_width, device=X.device).unsqueeze(-1)
                * beam_width
            )
    ...
    def forward_with_stop_emb(self, idx, tokenizer):
        """
        separate routine because of issues with torch DataParallel
